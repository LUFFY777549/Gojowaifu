...
async def single_tile(client, cq):
    await cq.answer()
    try:
...
async def single_cash(client, cq):
    await cq.answer()
    user_id = cq.from_user.id
    game = await load_game(user_id)
    if not game or game.get("mode") != "single":
...
async def mgame_challenge(client, message):
    """Usage: /mgame [bet] [@username or reply-to user]
    Creates a short challenge token and sends accept/decline buttons to target.
    """
...
@bot.on_callback_query(filters.regex(r"^mg:rej:"))
async def mg_reject(client, cq):
    await cq.answer()
    cid = cq.data.split(":")[2]
...
@bot.on_callback_query(filters.regex(r"^mg:acc:"))
async def mg_accept(client, cq):
    await cq.answer()
    cid = cq.data.split(":")[2]
    chal = pending_challenges.get(cid)
    if not chal:
...
@bot.on_callback_query(filters.regex(r"^mg:size:"))
async def mg_size_select(client, cq):
    await cq.answer()
    try:
        _,_,cid, size_str = cq.data.split(":")
        size = int(size_str)
    except:
...
@bot.on_callback_query(filters.regex(r"^mp:[0-9a-f]+:\d+$"))
async def mp_tile_press(client, cq):
    await cq.answer()
    try:
...
@bot.on_callback_query(filters.regex(r"^mpx:[0-9a-f]+:ign$"))
async def mp_ignore(client, cq):
    await cq.answer()
